...
.PHONY: help up prod stop down restart restart-container stop-container beauty composer logs bash psql redis test
...
help:
	...
	@echo ""
	@echo "🎭  PHP commands:"
	@echo "  make php <cmd>  - Run 'php <cmd>' inside the app container"
	@echo "  make beauty <cmd>  - Run './beauty <cmd>' inside the app container"
	@echo ""
	@echo "👀  PHPUnit:"
...
## 🎭 Run an beauty command (usage: make beauty migrate)
beauty:
	docker-compose exec -u www-data $(APP_CONTAINER) ./beauty $(filter-out $@,$(MAKECMDGOALS))
...
bash:
	docker-compose exec -u www-data $(APP_CONTAINER) sh
